...
function processGestureForDetection(results) {
    if (!results.gestures || results.gestures.length === 0 || results.gestures[0].length === 0) {
        // No hand detected during SHOOT phase
        playerFinalMove = null;
        document.getElementById("detectedText").innerHTML = "⚠️ No hand detected!";
        document.getElementById("detectedEmoji").innerHTML = "❌";
        document.getElementById('playerBox').classList.remove('detecting');
        return;
    }
    const gesture = results.gestures[0][0].categoryName;
    let newMove = null;
...
// ============================================================
// START BATTLE - UPDATED (Only Computer Shakes + No Random Choice)
// ============================================================
...
function startCameraBattle() {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            let finalPlayerMove = playerFinalMove;
            
            // If NO hand detected - show error message, don't randomly choose
            if (finalPlayerMove === null || finalPlayerMove === undefined) {
                // Stop computer shake
                computerHand.className = "";
                computerHand.style.transform = "translateY(0px) rotate(0deg)";
                ...
                // Show error message
                countdown.innerHTML = "⚠️ HAND NOT DETECTED!";
                computerText.innerHTML = "❌ No hand detected!";
                playerText.innerHTML = "❌ Please show your hand clearly!";
                ...
                return; // Exit the function - don't proceed with game
            }
